...
2. Допишете програмата по следния начин:
      2.1 Ако днес сме петък, програмата трябва да принти `"IT'S FRIDAY YET!"`
      2.2 Ако не сме петък, програмата трябва да принтира `"No, today is <current day>"`
      2.3 `<current day>` трябва да е string за днешния ден
